    ...
    pipe(src).map(
      composeMonoTypeFns(
        ...
        replaceWithNoMatchCheck(
          ...
          /predicate: \(value: bigint, index: number, array: BigInt64Array(?:<TArrayBuffer>)?\) => unknown/gu,
          `predicate: (value: bigint, index: number, array: ${typedArrayRef('BigInt64Array', config.tsLibShape, 'TArrayBuffer')}) => boolean`,
        ),
        replaceWithNoMatchCheck(
          // require predicate function to return boolean
          /predicate: \(value: bigint, index: number, array: BigUint64Array(?:<TArrayBuffer>)?\) => unknown/gu,
