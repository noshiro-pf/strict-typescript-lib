...
- `BLOCKED` — up to date, but a required check is failing or still pending. Go to
  step 3 or 4; do not rebase it, that only restarts the same matrix.
...
- Anything under `packages/v*/output/**` or `packages/v*/output-branded/**` —
  ...
  rebase, then run `pnpm ws:gen` and commit the result.
...
`type-check (ws:gen:with-codemod-fixed)` regenerates every supported TypeScript
version's lib files and is the long pole; the job carries a 30-minute timeout.
...
`repo-settings/rulesets/main.json` is that list, and it is shorter than what the
PR page shows. `type-check (ws:gen:with-codemod-fixed)` and `Validate PR title`
both run but are **not** required, so a red one there does not hold the merge —
fix it anyway, but do not report the PR as blocked on it.
...
run `pnpm run X` at the repository root, so `type-check (ws:type-check)`
reproduces as `pnpm run ws:type-check`. Three checks do not follow that rule:

| Check                        | What to run                                                     |
| :--------------------------- | :-------------------------------------------------------------- |
| `test-node-versions (<ver>)` | `pnpm run ws:type-check` on that Node                           |
...
There is no bundler build here — the "build" is the generator — so nothing needs
a `build` step before it runs. What does need care:

- **Every `style-check` and `type-check` job ends with `assert-repo-is-clean`.**
  So `fmt`, `codemod:full`, `lint:fix` and `ws:gen:with-codemod-fixed` fail by
  _changing_ files, and the fix is to run the command locally and commit what it
  wrote. A green run of the command with a dirty tree afterwards is still a
  failure.
...
- **`type-check (ws:type-check)` runs each version's lib-check.** To narrow it to
  the version that failed, use
  `pnpm --filter strict-ts-lib-vX.Y-source run type-check`, which runs `tsc@X.Y`
  over the generated lib with `skipLibCheck: false`.
- **`type-check (check:root)`** is the tooling type-check
  (`check:root:tooling` + `check:root:type`) over the generator, scripts and
...
`type-check` and `test-node-versions` jobs gate on
...
skill's two jobs is holding it — a pending Copilot review, a required check that
never reported, auto-merge disabled behind your back. Report that and stop. Do
not merge it.
